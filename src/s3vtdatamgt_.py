...
def get_file_list(url, username, password, aoi, startrecord):
    ...
    try:
        ...
        logger.info(
            str(
                [
                    ...
                    "--user=" + username,
                    "--password=" + password,
                    "--output-document=filelist.txt",
                    'https://scihub.copernicus.eu/dhus/search?q=footprint:"Intersects('
                    + aoi
                    + ')" AND platformname:Sentinel-3 AND producttype:SL_2_FRP___&rows=100&start='
                    + startrecord
                    + "&format=json",
                ]
            )
        ...
        logger.info(
            "Remote file listing process failed "
            + str(
                [
                    "wget",
                    "--no-check-certificate",
                    ...
                    "--password=" + password,
                    "--output-document=filelist.txt",
                    'https://scihub.copernicus.eu/apihub/search?q=footprint:"Intersects('
                    + aoi
                    + ')" AND platformname:Sentinel-3 AND producttype:SL_2_FRP___&rows=100&start='
                    + startrecord
                    + "&format=json",
        ...
        logger.info(
            ...
            + str(
                [
                    ...
                    "--user=" + username,
                    "--password=" + password,
                    "https://scihub.copernicus.eu/odata/v1/Products('"
                    + uuid
                    + "')/$value",
                ]
            )
...
def load_hotspots(
    filter_string, time_period, bbox, max_features, min_confidence, to_date
):
    y_max = bbox[0]
    x_min = bbox[1]
    y_min = bbox[2]
    ...
    if hotspots_gdf["confidence"][0] == None:
        ...
        hotspots_gdf = hotspots_gdf.loc[
            hotspots_gdf.confidence >= min_confidence
        ]
    ...
    logger.info(
        "Hotspots loaded successfully "
        + str(hotspots_gdf.geometry.total_bounds)
    )
    ...
    for configuration in cfg["configurations"]:
        ...
        if not os.path.exists("s3vt_inventory.json"):
            ...
            while startrecord <= upperlimit:
                ...
                with open("filelist.txt") as results:
                    for i in results:
                        responselist.append(json.loads(i))
        ...
        s3 = boto3.resource(
            "s3",
            aws_access_key_id=configuration["awskeyid"],
                ...
                for page in pages:
                    ...
                    try:
                        for entry in page["feed"]["entry"]:
                            ...
                            polygon = get_polygon_from_gml(
                                xmltodict.parse(entry["str"][1]["content"])[
                                    "gml:Polygon"
                        ...
                        logger.info(
                            "KeyError exception for get_polygon_from_gml()"
                        )
        s3vtgpd = pd.concat(frames)

        # Not sure why we need to index but do it anyway
        s3vtgpd = s3vtgpd.reset_index(drop=True)
        s3vtgpd["date"] = pd.to_datetime(
            s3vtgpd.summary.str.split(",", expand=True)[0].str.split(
                " ", expand=True
            )[1]
        )
                ...
                try:
                    subprocess.call(
                        [
                            ...
                            "s3",
                            "cp",
                            s3vtgpd.loc[i]["title"] + ".SEN3/",
                            "s3://s3vtaustralia/data/"
                            + folderdate
                            + "/"
                            + s3vtgpd.loc[i]["title"]
                            + ".SEN3/",
                    ...
                    subprocess.call(
                        [
                            ...
                            "s3",
                            "cp",
                            s3vtgpd.loc[i]["title"] + ".FRP.geojson",
                            "s3://s3vtaustralia/data/" + folderdate + "/",
                        ]
                    )
                except:
                    ...
                    subprocess.call(
                        ["rm", "-rf", s3vtgpd.loc[i]["title"] + ".SEN3"]
                    )
                    subprocess.call(
                        ["rm", "-rf", s3vtgpd.loc[i]["title"] + ".zip"]
                    )
