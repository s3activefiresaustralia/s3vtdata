...
def getredtoblack(number):
    ...
    redtoblack = (
        redorange + orangeyellow[1:] + yellowwhite[1:] + whiteblack[1:]
    )
...
def get_tles(tle_files):
    ...
    for url in tle_files:
        ...
        # TODO remote this section when https from sandbox is enabled.
        if Path(tle_file_basename).exists():
            continue
        ...
        try:
            download_file(url)
            logging.info(
                "Downloading URL from configuration sources:"
                + tle_file_basename
            )
        except OSError:
            logging.info(
                "Process could not download file:" + tle_file_basename
            )
            return ()
...
def clean_combine_tles():
    ...
    # clean up the lines
    tles = [
        (tles[i], tles[i + 1], tles[i + 2]) for i in range(0, len(tles) - 2, 3)
    ]
    return tles


def check_tle_is_current(tle_files):
    ...
    if os.path.exists("tles.txt"):
        tle_retrieve_time = datetime.fromtimestamp(
            os.path.getmtime("tles.txt")
        )
    else:
        get_tles(tle_files)
    ...
    if tle_timesinceretrieve > timedelta(hours=24):
        ...
        get_tles(tle_files)
        logging.info(
            "Stale TLE detected and replaced at" + str(tle_retrieve_time)
        )
    else:
...
def get_vector_file(
    attributes, input_points, poly_or_line, ogr_output, ogr_format
):
    ...
    """
    field_definition = ogr.FieldDefn(
        "Acquisition of Signal Local    :", ogr.OFTString
    )
    field_definition.SetWidth(30)
    layer.CreateField(field_definition)
    field_definition = ogr.FieldDefn(
        "Acquisition of Signal UTC    :", ogr.OFTString
    )
    field_definition.SetWidth(30)
    layer.CreateField(field_definition)
    field_definition = ogr.FieldDefn(
        "Loss of Signal UTC           :", ogr.OFTString
    )
    field_definition.SetWidth(30)
    layer.CreateField(field_definition)
    field_definition = ogr.FieldDefn(
        "Transit time                 :", ogr.OFTString
    )
    field_definition.SetWidth(30)
    layer.CreateField(field_definition)
    field_definition = ogr.FieldDefn(
    ...
    feature.SetField(
        "Orbit height                 :", attributes["Orbit height"]
    )
    ...
    """
    feature.SetField(
        "Acquisition of Signal Local    :", attributes["Local time"]
    )
    feature.SetField(
        "Acquisition of Signal UTC    :", str(attributes["AOS time"])
    )
    feature.SetField(
        "Loss of Signal UTC           :", str(attributes["LOS time"])
    )
    feature.SetField(
        "Transit time                 :", str(attributes["Transit time"])
    )
...
def get_effective_heading(
    ...
    xfac = math.sqrt(
        1 - e * (2 - e) * (math.pow(math.sin(math.radians(latitude)), 2))
    )
    phi_rad = math.asin(
    ...
    n = r / math.sqrt(
        1 - e * (math.pow(math.sin(math.radians(latitude)), 2))
    )  # N
    altphi_rad = (
        latitude
        - 180
        * math.asin(
            n * e * math.sin(lat_rad) * math.cos(lat_rad) / orbit_radius
        )
        / math.pi
    )  # Alt Phi0'(Radians)
    rho_rad = math.acos(
...
def folium_timespan_geojson_html(
    schedule,
    satname,
    output_path,
    ground_station_name
):
    timespan_map = folium.Map(
        location=[-26, 132], tiles="OpenStreetMap", zoom_start=3
    )
    lines = []
    polygons = []
    ...
    for i in schedule:
        ...
        # folium expects a time for each point - could use iterate for len of points and time period to
        # get times per point - or add to dict from original function which uses time
        lines.append(
            {
    ...
    foliumtimespanhtml = os.path.join(
        output_path, satname + "." + ground_station_name + ".timespan.map.html"
    )
...
def get_upcoming_passes(
    ...
    for tle in tles:
        ...
        if tle[0] == satellite_name:
            ...
            while lostime <= (
                passes_begin_time + timedelta(minutes=passes_period)
            ):
                ...
                orbitnumber = (
                    orb.get_orbit_number(to_datetime(rt)) + orbitoffset
                )
                ...
                try:
                    ...
                    tag = repo.tags[
                        len(repo.tags) - 1
                    ].tag.tag  # TODO work out if git repo, if not skip TRY
                except:
                    tag = "0.0.0"
                print("code tag             = ", tag)
                print("------------------------------------------")
                ...
                while deltatime < to_datetime(st):
                    ...
                    effectiveheading = Geodesic.WGS84.Inverse(
                        subsatlat1, subsatlon1, subsatlat, subsatlon
                    )["azi1"]
                ...
                if not any(
                    d["SWATH_FILENAME"] == attributes["SWATH_FILENAME"]
                    for d in schedule
                ):
                    ...
                    if (
                        imagingnode == "both"
                        or imagingnode == attributes["Node"]
                    ) and (
                        to_datetime(rt)
                        < (
                            passes_begin_time
                        ...
                        add_layer_to_map(
                            ORBIT_FILENAME,
                            satname + "." + str(orbitnumber) + ".orbit",
                            "red",
                        )

                        # Create a temporal KML
                        pol = kml.newpolygon(
                            name=satname + "_" + str(orbitnumber),
                        ...
                        rt_kml = datetime.strptime(
                            str(rt), "%Y/%m/%d %H:%M:%S"
                        )
                        st_kml = datetime.strptime(
                            str(st), "%Y/%m/%d %H:%M:%S"
                        )
                ...
                observer.date = lostime + timedelta(minutes=90)
                lostime = datetime.strptime(
                    str(observer.date), "%Y/%m/%d %H:%M:%S"
                )

    kml.save(
        os.path.join(output_path, satname + "." + ground_station_name + ".kml")
    )
    ...
    for acquisition in schedule:
        attributeshtml.append(
            "<td>" + acquisition["Satellite name"] + "</td>"
            '<td><a href="'
            + str(
                os.path.relpath(
                    acquisition["Swath filename"], os.path.dirname(foliumhtml)
                )
            )
            + '">'
            + str(acquisition["Sensor code"])
            + "</a></td>"
            '<td><a href="'
            + str(
                os.path.relpath(
                    acquisition["Orbit filename"], os.path.dirname(foliumhtml)
                )
            )
            + '">'
            + str(acquisition["Orbit"])
            + "</a></td>"
    ...
    renderedoutput = template.render(
        ...
        foliumhtml=os.path.relpath(foliumhtml, output_path),
        satelliteslist=satelliteslist,
    )

    with open(
        os.path.join(
            output_path, satname + "." + ground_station_name + ".schedule.html"
        ),
        "w",
    ) as fh:
...
def main(
    configuration_file: Optional[Union[Path, str]] = "s3vtconfig.yaml",
    start_datetime: Optional[str] = None,
    period: Optional[int] = None,
    output_path: Optional[Union[Path, str]] = None,
):
    """Main process to run satellite swath genrations workflow.

    :param configuration_file: The path to a configuration file.
    :param start_datetime: The start_datetime to generate satellite swaths.
    :param period: The time period to generate the satellite swath.
    :param output_path: The path to store the swath files.
    ...
    # Get configurations
    satellites = []
    with open(Path(configuration_file).as_posix(), "r") as config:
        cfg = yaml.load(config, Loader=yaml.Loader)

    for satellite in cfg["missions"]:
        satellites.append(satellite["satellite"]["tle"])

    tle_files = cfg["tlefiles"]["url"]
    tles = check_tle_is_current(tle_files)
    ...
    if start_datetime is None:
        ...
        start = datetime.strptime(start_datetime, "%Y-%m-%dT%H:%M:%SZ")
    ...
    ground_station = cfg["scenario"][
        "ground_station"
    ]  # ('-23 42', '133 54')  # Alice Spring Data Acquisition Facility
    ground_station_latitude, ground_station_longitude = ground_station
    map_centre_lat = (
        ground_station_latitude.split()[0]
        + "."
        + str(int((float(ground_station_latitude.split()[1]) / 60) * 100))
    )
    map_centre_lon = (
        ground_station_longitude.split()[0]
        + "."
        + str(int((float(ground_station_longitude.split()[1]) / 60) * 100))
    )
    ...
    if output_path is None:
        output_path = os.path.relpath(
            cfg["outputs"]["output_path"], os.getcwd()
        )
    ...
    # while 1: do this without stopping - comment out to do once - new scenario could
    # be run once each 24hr period, then only update satellite positions each minute
    ...
    for satellite_name in satellites:
        for satellite in cfg["missions"]:
            ...
            satellite_map = folium.Map(
                location=[float(map_centre_lat), float(map_centre_lon)],
                tiles="OpenStreetMap",
                zoom_start=3,
            )

            if satellite["satellite"]["tle"] == satellite_name:
        ...
        logging.info("Get upcoming passes for " + satellite_name)
        logging.info(str([satellite_name, start, period]))
        get_upcoming_passes(
            satellite_name,
            start,
            period,
